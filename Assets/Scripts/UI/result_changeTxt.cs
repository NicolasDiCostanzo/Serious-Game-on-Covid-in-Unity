...
public class result_changeTxt : MonoBehaviour
{
    ...
    public void ChangeTxt()
    {
        ...
        {
            if (mistakesPanel.transform.childCount == 0)
            {
                Debug.Log("aucune erreur !");
            }
    ...
    void AllMissedPlacedMembers()
    {
        foreach (MedicalInfoHolder member in GameManager.Earth)
        {
            if (member.patientAffliction.ToString() != "Covid")
            {
                ...
                description.text = FormateMistakesDisplaying(member, "Earth");
        ...
        {
            if (member.patientAffliction.ToString() != "OtherDisease")
            {
                GameObject go = new GameObject("patientInfo");
                go.transform.parent = mistakesPanel.transform;
                TextMeshProUGUI description = go.AddComponent<TextMeshProUGUI>();
                description.text = FormateMistakesDisplaying(member, "the station");
                description.fontSize = 25;
            }
        }
        ...
        {
            if (member.patientAffliction.ToString() != "Healthy")
            {
                GameObject go = new GameObject("patientInfo");
                go.transform.parent = mistakesPanel.transform;
                TextMeshProUGUI description = go.AddComponent<TextMeshProUGUI>();
                description.text = FormateMistakesDisplaying(member, "mission");
                description.fontSize = 25;
            }
        }
    }
